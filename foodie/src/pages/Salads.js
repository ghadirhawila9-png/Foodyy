...
function Salads() {
  ...
  useEffect(() => {
    const savedRecipes = localStorage.getItem("saladsRecipes");
    if (savedRecipes) {
  ...
  const handlePost = () => {
    ...
    localStorage.setItem("saladsRecipes", JSON.stringify(updatedRecipes));
    setNewRecipe("");
  ...
  return (
    <div className="salads-page">
      <NavBar />
      <h1 className="page-title">Salads Recipes 🥗</h1>
            ...
            {recipe.content.length > 0 && (
              <div className="recipe-content">
                {recipe.content.map((line, i) => (
                  <p key={i} className="recipe-line">🥗 {line}</p>
                ))}
              </div>
            )}
