...
function Soups() {
  ...
  useEffect(() => {
    const savedRecipes = localStorage.getItem("soupsRecipes");
    if (savedRecipes) {
  ...
  const handlePost = () => {
    ...
    localStorage.setItem("soupsRecipes", JSON.stringify(updatedRecipes));
    setNewRecipe("");
  ...
  return (
    <div className="soups-page">
      <NavBar />
      <h1 className="soups-page-title">Soups Recipes 🍲</h1>
      ...
      <div className="soups-recipes-feed">
        {filteredRecipes.map((recipe, index) => (
          <div key={index} className="soups-recipe-card">
            <h2 className="soups-recipe-title">{recipe.title}</h2>
            {recipe.content.length > 0 && (
              <div className="soups-recipe-content">
                {recipe.content.map((line, i) => (
                  <p key={i} className="soups-recipe-line"> 🍲{line}</p>
                ))}
              </div>
            )}
